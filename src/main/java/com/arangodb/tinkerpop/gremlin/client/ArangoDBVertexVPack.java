...
/**
 * The ArangoDBVertexSerialiser is a specialized VPackSerializer that can traverse the vertex vertexProperties and store both
 * their values and their expected Java types, as well as the property's cardinality and nested vertexProperties. In order
 * to avoid breaking the expectations of the serialized documents, the Tinkerpop metadata will be stored separately
 * under the "!tinkerpop" property. This allows reusing existing collections with Tinkerpop and quering the documents
 * va AQL without having to worry about the Tinkerpop metadata.
 *
 * We will use the following JSON structure to persist the Tinkerpop metadata. Note that a property value is a collection
 * (this way we can accommodate all cardinalities in a similar fashion). For this reason the pproperties value is a list
 * of objects, where each object represents the vertexProperties of one element of the values pvalue collection. Similarly,
 * the ptype value is also a list. For nested vertexProperties this is not the case as only singled valued vertexProperties are
 * allowed.
 * <pre>{@code
 *   {
 *     ...
 ...
 *             "key": "<npname>"
 *             "value": "<proppropValue>"
 *             "type:   "<proppropValue.class.qualifiedName()>"
 *           },
 *           { ... }
 *           ...
 *         ]
...
public class ArangoDBVertexVPack implements VPackSerializer<ArangoDBVertex>, VPackDeserializer<ArangoDBVertex> {
    ...
    public void serialize(
        ...
        // Iterator<? extends Property<Object>> itty = value.properties(value.keys().toArray(new String[0]));
    ...
    public ArangoDBVertex deserialize(VPackSlice parent, VPackSlice vpack, VPackDeserializationContext context) throws VPackException {
        ...
        while (it.hasNext()) {
            ...
            if (key.equals(TINKERPOP_METADATA_KEY)) {
                ...
                // TODO We need to handle ID types?
                Method method = null;
                try {
                ...
                if (md == null) {
                    ...
                    if (cardinality.equals(VertexProperty.Cardinality.single)) {
                        ArangoDBVertexProperty<?> vp = new ArangoDBVertexProperty<>(key, rawValue, vertex, cardinality);
                        vertex.attachProperties(key, Collections.singletonList(vp));
                    }
